package com.jntucep.c19_delhi.VerticalMarqueeTextView;
	...
	private class AutoScrollTextView extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			while (!stop) {
				...
				if (!(VerticalMarqueeTextView.this).isPressed() && isUserScrolling && !isPaused) {
					...
					activity.runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							if ((VerticalMarqueeTextView.this).isPressed()) {
								...
								if ((VerticalMarqueeTextView.this).getScrollY() >= pixelCount - 350) {
